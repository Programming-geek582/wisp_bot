...
<:reverse:879724816834375791> Reversed text: {text[::-1]}
    ...
    async def balls(self, ctx : commands.Context):
        balls = ['balls', 'footballs', 'cherry', 'basketballs', 'volleyballs']
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://api.giphy.com/v1/gifs/search?api_key=R51RiLuUfTTG4AGonaXsV7N6kuHaGRXF&q={random.choice(balls)}&limit=25') as request:
                json = await request.json()
        await ctx.send(json['data'][0]['images']['original']['url'])
