...
class Image(commands.Cog, name="image"):
    ...
    @commands.command(help="Get the screenshot of a website")
    @commands.cooldown(1, 30, BucketType.member)
    async def screenshot(self, ctx : commands.Context, url : str = None):
        if url == None:
            return await ctx.send('Argument `url` cannot be none`')
			
        embed = nextcord.Embed(title="Screenshot", colour=0x030014)
        embed.set_image(url=f"https://api.popcat.xyz/screenshot?url={url}")
        await ctx.send(embed=embed)
